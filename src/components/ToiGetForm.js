...
export default function ToiGetForm() {
  ...
  function insertOpeningHours() { // Retorno do componente HorarioSemana
  ...
  function updateOpeningHours(openingHours) { // Retorno do componente HorarioSemana
    if (openingHours) {
  ...
  function newId() { // Usado na inclusão de um novo banheiro
    let lastId = 0;
    for (let i = 0; i < toiletList.length; i++) {
      if (toiletList[i].id > lastId) {
        lastId = toiletList[i].id;
      }
    }
    lastId++;
    return lastId;
  ...
  function handleSubmit(e) { // Tratamento do form
    let validated =
